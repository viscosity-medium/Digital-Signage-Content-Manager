...
export const getDirectoryTotalDuration = ({item}: {item: ScheduleFolderInterface}) => {
...
export const getFileItemDuration = ({ innerItem }: { innerItem: ScheduleItemInterface }) => (typeof innerItem.limits.time === "string" && innerItem.limits.time !== "default") ?
    `(${innerItem.limits.time.replace(/.*T\d{2}:|\..*$/gm, "")})` : "";